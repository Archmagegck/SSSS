...
public:
	...
	CbyteShare(unsigned short _m,unsigned short _n) {m=_m;n=_n;};	//初始化m，n
...
int main(int argc, char* argv[])
{
	...
	int m=255,n=255,msg,i;	//int占4个字节,32位
	...
	{
		allshares[i-1][0]=i; 
		printf("\t%d", allshares[i-1][1]=mshare.GetShares(i));
	};
	printf("\n");
...
//	mshare.makeShare();
	...
	for(i=1;i<=m;i++) 
	{		
		torecovery[i-1][0]=tmprand[i-1]+1;
		torecovery[i-1][1]=allshares[tmprand[i-1]][1];
		printf("\t %d：%d", torecovery[i - 1][0], torecovery[i - 1][1]);
	}
	printf("\n");

	printf("\nm=====%d ",mshare.RestorShareByShares(torecovery)); 
