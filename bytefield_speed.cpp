...
public:
	...
	void SetShareByte(word8 bytemsg) {toShareBytem=bytemsg;RandSetAx();setv();Ax[0]=toShareBytem; 	};
...
*/		printf("\n---share--------------------\n");
	for(i=0;i<n;i++)
	{
		...
		for(j=0;j<m;j++)
		{
			Ay[i]=Ay[i]+v[i][j]*Ax[j];
		}
		v[i][j]=Ay[i]; //give v[i][m]=ay[i]

		printf(" %3d ",Ay[i].value());
	}
	printf("\n-------share end------------");
...
for(i=0;i<TIMES*4;i++)
 ...
 printf( "\nmt8=%qi  %u\nd",timeMakeShare,timeMakeShare/TIMES );
timeMakeShare=0;
