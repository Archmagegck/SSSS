...
public:
	...
	inline Cbytefield operator+ (Cbytefield b){return a^b.value();};
	inline Cbytefield operator- (Cbytefield b){return a^b.value();};
	inline Cbytefield operator* (Cbytefield b){return mul(a,b.value());};
	inline Cbytefield operator/ (Cbytefield b){return mul(a,inv(b.value()));};
	inline Cbytefield operator= (Cbytefield b){return a=b.value();};
	inline int operator== (Cbytefield b){return a==b.value();};
	inline int	operator< (Cbytefield b){return a<b.value();};
	inline Cbytefield invert() {return inv(a);};
...
public:
	...
	word8 RestorShareByShares(word8 shares[][2]);

	inline word8 GetShares(int i) {return yn[i-1][1].value();}
		
	

};

word8  CbyteShare::RestorShareByShares(word8 shares[][2])
{
	...
	for(i=0;i<m;i++)
	{v[i][m]=shares[i][1]; // printf("%3d,",v[i][m].value()); 
 	}
...
int main(int argc, char* argv[])
{
	...
	int m=255,n=255,msg,i;
	word8 	 allshares[255][2],torecovery[255][2],tmprand[255];
	m=atoi(argv[1]);
	n=atoi(argv[2]);

	printf("m=%d n=%d argv[3]=%s, argv[4]=%s",m,n,argv[3],argv[4]);

	
	if(strcmp(argv[3],"make")==0)
	{
		FILE *fpin=NULL, *fpout[255]={0};
		fpin=fopen(argv[4],"rb");
		
		for(i=1;i<=n;i++)
		{
			char tmpfilename[1024];
			sprintf(tmpfilename,"%s_%d.share",argv[4],i);
			
			fpout[i-1]=fopen(tmpfilename,"wb");
			fputc(i,fpout[i-1]);
		}		
		...
		while(!feof(fpin))
		{
			word8 msg=fgetc(fpin);
	
			mshare.SetShareByte(msg);

			for(i=1;i<=n;i++) {fputc(mshare.GetShares(i),fpout[i-1]);};
		}
	...
	if(strcmp(argv[3],"recovery")==0)
	{
		...
		FILE *fpout=NULL, *fpin[255]={0};

		word8 torecovery[255][2];
		...
		for(i=1;i<=255;i++)
		{
			...
			if( (fpin[k]=fopen(tmpfilename,"rb"))==NULL) 
			{
				continue;
			}
			else
			{	torecovery[k][0]=fgetc(fpin[k]);
				if(k>=m-1) break;
				k++;
			}
		...
		if(k<m-1) 
		{
			printf("no enough shares");
			for(i=0;i<=255;i++)
			{
				if(fpin[i]) fclose(fpin[i]);
			}
		...
		char tmpfilename[1024];
		sprintf(tmpfilename,"%s.recovery",argv[4]);

		fpout=fopen(tmpfilename,"wb");
		...
		while(!feof(fpin[0]))
		{
			for(k=0;k<m;k++) {torecovery[k][1]=fgetc(fpin[k]);};

			word8 rmsg=mshare.RestorShareByShares(torecovery);
